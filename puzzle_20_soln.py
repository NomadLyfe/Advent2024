...
for trailhead in trailheads:
    ...
    next_positions = [[(a + c, b + d) for (a, b), (c, d) in zip(directions, [(curr_position)]*4) if 0 <= a+c < len(grid[0]) and 0 <= b+d < len(grid) and grid[b+d][a+c] == curr_value + 1]]
    ...
    while len(next_values[i]) > 0:
        i += 1
        if i == 9:
            p1 += len(next_positions[-1])
            break
        for dir in next_positions[i-1]:
            curr_position = dir
            curr_value = grid[curr_position[1]][curr_position[0]]
            next_positions[i] += [(a + c, b + d) for (a, b), (c, d) in zip(directions, [(curr_position)]*4) if 0 <= a+c < len(grid[0]) and 0 <= b+d < len(grid) and grid[b+d][a+c] == curr_value + 1]
